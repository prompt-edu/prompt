...
2. **Reuse over custom:** UI built from `@tumaet/prompt-ui-components` (e.g. `PromptTable`,
   `ManagementPageHeader`, `DeleteConfirmation`) instead of re-implementing; shared state, types,
   hooks, and queries from `@tumaet/prompt-shared-state`. Flag custom code that duplicates an
...
3. **Data/state:** server state via React Query (`useQuery`/mutations with key invalidation + toast
   feedback) using the shared hooks/queries; global state via Zustand stores; `axiosInstance` from
   `@tumaet/prompt-shared-state` (not raw axios) for calls.
