...
export async function getTeams(api: APIRequestContext): Promise<Team[]> {
  const res = await api.get(teamsUrl())
  if (!res.ok()) {
...
export async function deleteTeamByName(name: string) {
  const admin = await apiContextFor('admin')
  try {
    const team = (await getTeams(admin)).find((t) => t.name === name)
    if (team) {
      await admin.delete(`${teamsUrl()}/${team.id}`)
    }
  } finally {
    await admin.dispose()
  }
