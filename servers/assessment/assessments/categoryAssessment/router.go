...
func RegisterRoutes(routerGroup *gin.RouterGroup, service *CategoryAssessmentService, guard assessmentGuard, authMiddleware func(allowedRoles ...string) gin.HandlerFunc) {
	...
	r.POST("", audit.Describe("Saved a category assessment"), authMiddleware(promptSDK.PromptAdmin, promptSDK.CourseLecturer, promptSDK.CourseEditor), guard.RequireAssessmentEnabled(), service.createOrUpdateCategoryAssessment)
}
