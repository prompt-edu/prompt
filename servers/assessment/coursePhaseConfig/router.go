...
func RegisterRoutes(routerGroup *gin.RouterGroup, service *CoursePhaseConfigService, authMiddleware func(allowedRoles ...string) gin.HandlerFunc) {
	...
	coursePhaseRouter.POST("/release", audit.Describe("Released the assessment results"), authMiddleware(promptSDK.PromptAdmin, promptSDK.CourseLecturer), service.releaseResults)
	coursePhaseRouter.POST("/unrelease", audit.Describe("Unreleased the assessment results"), authMiddleware(promptSDK.PromptAdmin, promptSDK.CourseLecturer), service.unreleaseResults)
