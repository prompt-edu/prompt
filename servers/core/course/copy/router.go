...
// RegisterRoutes mounts the course copy endpoints on the given router group.
// @Summary Course Endpoints
// @Description Endpoints for managing courses and course graphs
// @Tags courses
// @Security BearerAuth
func RegisterRoutes(router *gin.RouterGroup, service *CourseCopyService, authMiddleware func() gin.HandlerFunc, checkCoursePermission permissionValidation.PermissionCheck) {
	setupCourseCopyRouter(router, service, authMiddleware, permissionValidation.CheckAccessControlByRole, checkAccessControlByIDWrapper(checkCoursePermission))
}
...
func setupCourseCopyRouter(router *gin.RouterGroup, s *CourseCopyService, authMiddleware func() gin.HandlerFunc, permissionRoleMiddleware, permissionIDMiddleware func(allowedRoles ...string) gin.HandlerFunc) {
	course := router.Group("/courses", authMiddleware())

	course.GET("/:uuid/copyable", permissionIDMiddleware(permissionValidation.PromptAdmin, permissionValidation.CourseLecturer), s.checkCourseCopyable)
	course.POST("/:uuid/copy", permissionIDMiddleware(permissionValidation.PromptAdmin, permissionValidation.CourseLecturer), s.copyCourse)
}
...
func (s *CourseCopyService) copyCourse(c *gin.Context) {
