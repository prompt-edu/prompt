...
-- (resource_instance_member -> resource_instance -> resource_config -> provider_config):
-- an explicit delete per root table keeps the handler correct if a migration ever weakens
-- one of those cascades, and the tests count the raw tables so a weakened cascade fails
-- loudly.

-- name: DeleteResourceInstanceMembersByCoursePhase :exec
DELETE FROM resource_instance_member
WHERE resource_instance_id IN (
    SELECT id FROM resource_instance WHERE course_phase_id = $1
);
