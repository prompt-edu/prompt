...
-- name: GetPresentationForShare :one
SELECT * FROM presentation WHERE id = $1 AND course_phase_id = $2 FOR SHARE;

-- name: LockPresentationsByPhase :many
...
-- name: ListMaterialStorageKeysByPresentation :many
...
-- name: ReclaimExpiredPendingMaterials :many
WITH reclaimed AS (
...
-- name: GetPrivacyFeedbackContributions :many
SELECT c.* FROM feedback_contributor c WHERE c.user_id = $1;

-- name: GetPrivacyFeedbackAnswers :many
SELECT a.* FROM feedback_answer a WHERE a.updated_by_user_id = $1;
